...
`scripts/trading-engine.ts` still routes live entries through `TRADE_SIZE = 1`. Position size is not yet wired to edge magnitude, model confidence, or asset volatility. The additive foundation now exists in `scripts/lib/trading/sizing.ts`: binary-outcome fractional Kelly sizing with caps, confidence scaling, cash limits, and tests. `candidate-actions.json` now exposes read-only `sizingPreviews` so dry runs can show what fractional-Kelly sizing would do before live size mutation. Probability sourcing now prefers resolved calibration buckets, then signal/asset history, then signal-family history, then `signal.confidence` fallback. **The next step is reviewing preview output by signal family and adding portfolio exposure/correlation previews before changing live size.**
...
- Foundation shipped: `scripts/lib/trading/sizing.ts` exposes tested binary fractional-Kelly sizing and probability resolution. `candidate-actions.json` now includes read-only sizing previews sourced from calibration/history/fallback evidence, but live entry size remains flat so fixture gates can separate metadata from behavior changes.
