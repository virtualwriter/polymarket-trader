...
describe("scanner output helpers", () => {
  ...
  it("builds valuation CSV rows with existing rounding and percent conversions", () => {
    const row = buildScannerValuationCsvRow({
      ...
      oilPcRatio: 0.4321,
      hyperliquidQuotes: {
        HYPE: { fundingAnnualized: 0 },
        MU: { fundingAnnualized: 0.04567 },
        "GOLD (GC)": { fundingAnnualized: 0.015 },
        "OIL (CL)": { fundingAnnualized: -0.125 },
      },
      spySpot: 6123.456,
    ...
    expect(row).toMatchObject({
      ...
      hype_spot: 38.1235,
      hype_hl_funding_ann: 0,
      gold_gld_spot: null,
      gold_hl_funding_ann: 1.5,
      mu_hl_funding_ann: 4.57,
