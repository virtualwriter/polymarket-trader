...
STANDING RESEARCH PRIORITY (operator-set, 2026-08-25):
...
PANEL_* findings (clusterKey starting with PANEL_NO_ or PANEL_YES_) come from the outcome panel: every archived Polymarket contract-day with measured forward contract returns — NOT from engine shadow trades. Their win rates are already tested against the pool's own base rate (a NO at 80c winning 80% of the time is zero edge), and they have passed a temporal holdout on recent unmined days. When authoring from a PANEL finding: (a) copy the conditions shown in its title verbatim as your hypothesis conditions — they are already valid catalog keys; (b) the prediction MUST be about the contract position's P&L over the stated horizon (e.g. "buying NO on these contracts is profitable over 3 days"), not about spot movement; (c) set timeframeDays to the horizon in the clusterKey (PANEL_NO_3D means 3 days); (d) direction is "neutral" unless the thesis genuinely carries a spot view.
SPOTPANEL_* findings (clusterKey starting with SPOTPANEL_LONG_ or SPOTPANEL_SHORT_) come from the spot outcome panel: every (asset, day) in the valuation history with measured forward SPOT returns. Their win rates are counted on the engine's own exam (a win = a move of >= the stated exam threshold in the thesis direction) against the pool's empirical base rate at that same exam, with a temporal holdout. When authoring from a SPOTPANEL finding, the exam alignment is NOT optional: (a) copy its suggestedConditions verbatim (they include the asset scope and per-asset derived keys); (b) set direction explicitly to the side in the clusterKey (SPOTPANEL_SHORT_3D means direction "short"); (c) your prediction MUST contain the exam threshold as "> X%" exactly as shown in the finding title (e.g. "declines > 1%") — that string sets the bar the scorer grades against, and omitting it silently changes the exam to 2%; (d) set timeframeDays to the horizon in the clusterKey; (e) a 7-day spot thesis tests serially and cannot reach 20 tests inside the verdict cap alone — author it as 2-3 sibling variants with distinct conditions (same originFindingId) or prefer the 1d/3d horizon variants.
${opportunityLines}
