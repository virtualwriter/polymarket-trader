...
import {
  ...
  type ShadowOutcome,
} from "./lib/trading/auto-promotion.js";
...
const BLOCKED_SIGNAL_OBSERVATION_CONFIG = {
...
interface SetupTruthRecord {
  ...
  evidenceSummary: {
    ...
    /** Dispersion of per-trade P&L, enabling the expectancy route to eligibility. */
...
function generateOneTouchHighEdgeNoSignals(
  ...
  // Deliberately the shadow gate, not the old strict |edge|>=15pt rule. The
  // evidence that authorized this promotion was generated by
  // oneTouchNoShadowEligible, so live entries must use the same predicate —
  // promoting on one rule and trading another is the same class of mistake as
  // scoring a hypothesis short and opening it long.
  ...
  for (const row of candidates) {
    const edgeMagnitude = sellYesEdgePts(row) ?? Math.abs(row.edgePts ?? 0);
    const event = latestSnapshot.polymarket.find((candidate) => candidate.slug === row.eventSlug);
    const contract = event?.contracts.find((candidate) => candidate.marketId === row.marketId);
    ...
    const strength = Math.min(1, edgeMagnitude / 30) * sizeFraction;
    const highConviction = edgeMagnitude >= ONE_TOUCH_HIGH_EDGE_CONVICTION_EDGE;
    const sizeNote = sizeFraction < 1 ? ` Sized at ${(sizeFraction * 100).toFixed(0)}% while proving out live.` : "";
    ...
    signals.push({
      ...
      thesis: `[ONE-TOUCH NO LIVE, auto-promoted] Fade upside barrier on ${row.asset} at ${edgeMagnitude.toFixed(1)}pt sell-YES edge (FIND-0020 gate: highs only, >=${ONE_TOUCH_NO_SHADOW_MIN_ENTRY_EDGE_PTS}pt); hold ${ONE_TOUCH_HIGH_EDGE_HOLD_DAYS}d for repricing.${highConviction ? " Edge >=20pt: high-conviction bucket." : ""}${sizeNote}`,
      hypothesisId: null,
...
/**
 * True when a one-touch shadow satisfies the FIND-0020 entry rule: fade only
 * upside barriers, and only when the gap is worth fading.
 *
 * Read from the heatmap row frozen at open, so it classifies historical
 * shadows by the rule that governs entries today rather than by whatever gate
 * happened to be in force when they were taken.
 */
function oneTouchShadowPassesFindGate(shadow: BlockedSignalShadow): boolean {
  const row = shadow.heatmapRowSnapshot?.row;
  if (!row) return false;
  const direction = String(row.direction ?? "").trim();
  if (direction !== ONE_TOUCH_FADE_ALLOWED_DIRECTION) return false;
  const edge = Number(String(row.sell_yes_edge_pts ?? "").trim());
...
function setupIdForShadow(shadow: BlockedSignalShadow): { setupId: string; setupLabel: string } {
  if (shadow.blockedReason === "one_touch_high_edge_shadow") {
    ...
    // history it reads 42% / +0.37% and can never promote; on its own it is
    // 57% / +5.81%. Same trades, different granularity — the edge is only
    // visible when the family boundary matches the entry rule.
    if (shadow.signalType === ONE_TOUCH_HIGH_EDGE_SIGNAL_NO && oneTouchShadowPassesFindGate(shadow)) {
      return {
        setupId: "one_touch_high_edge_no_gated",
        setupLabel: "One-touch NO fade, upside barriers with a large gap (FIND-0020)",
      };
    }
...
//
// Replaces the hand-flipped ENABLE_ constants. A family listed here turns
// itself on once its own resolved shadows clear the bars in auto-promotion.ts,
// and every evaluation is written to auto-promotions.json so the decision is
// auditable after the fact. Demotion stays with the realized-P&L kill switch in
// signal-pnl-guard.ts, which operates on live trades.
// ---------------------------------------------------------------------------

const AUTO_PROMOTION_STATE_FILE = "auto-promotions.json";

interface AutoPromotableFamily {
  setupId: string;
  signalType: string;
  label: string;
  /**
   * When this family's entry rule last changed. Shadows opened before it were
   * the data used to choose the rule, so they cannot also serve as its
   * out-of-sample validation.
   */
  gateInstalledAt: string;
}

const AUTO_PROMOTABLE_FAMILIES: AutoPromotableFamily[] = [
  {
    setupId: "one_touch_high_edge_no_gated",
    signalType: ONE_TOUCH_HIGH_EDGE_SIGNAL_NO,
    label: "One-touch NO fade, upside barriers with a large gap (FIND-0020)",
...
interface AutoPromotionStateEntry {
  ...
  reason: string;
  evidence: ReturnType<typeof summarizeShadowOutcomes>;
  liveTradesSincePromotion: number;
  history: Array<{ at: string; event: "promoted" | "blocked"; reason: string }>;
}

interface AutoPromotionState {
  schemaVersion: number;
  updatedAt: string;
  families: Record<string, AutoPromotionStateEntry>;
}

function readAutoPromotionState(): AutoPromotionState {
  return readJson<AutoPromotionState>(AUTO_PROMOTION_STATE_FILE, {
    schemaVersion: 1,
    updatedAt: new Date().toISOString(),
    families: {},
  });
...
/**
 * Re-score every auto-promotable family against its resolved shadows.
 *
 * Promotion is sticky: once a family goes live it is not un-promoted by a
 * shadow-evidence wobble, because live P&L is the better signal at that point
 * and the kill switch already watches it.
 */
function evaluateAutoPromotions(
  blockedSignals: BlockedSignalShadow[],
  closedTrades: ClosedTrade[],
): AutoPromotionState {
  const state = readAutoPromotionState();
  const now = new Date().toISOString();

  const outcomesBySetup = new Map<string, ShadowOutcome[]>();
  for (const shadow of blockedSignals) {
    ...
    else outcomesBySetup.set(setupId, [outcome]);
  }

  for (const family of AUTO_PROMOTABLE_FAMILIES) {
    const evidence = summarizeShadowOutcomes(outcomesBySetup.get(family.setupId) ?? [], family.gateInstalledAt);
    const decision: AutoPromotionDecision = evaluateAutoPromotion(evidence);
    const prior = state.families[family.setupId];
    const alreadyLive = prior?.promoted === true;
    const promoted = alreadyLive || decision.promote;

    const liveTradesSincePromotion = prior?.promotedAt
      ? closedTrades.filter((t) => t.signalType === family.signalType && t.closedAt >= prior.promotedAt!).length
      : 0;

    const history = prior?.history ?? [];
    if (decision.promote && !alreadyLive) {
      history.push({ at: now, event: "promoted", reason: decision.reason });
    } else if (!promoted && (prior?.reason ?? "") !== decision.reason) {
      history.push({ at: now, event: "blocked", reason: decision.reason });
    }

    state.families[family.setupId] = {
      setupId: family.setupId,
      signalType: family.signalType,
      label: family.label,
      promoted,
      promotedAt: alreadyLive ? prior!.promotedAt : (decision.promote ? now : null),
      lastEvaluatedAt: now,
      reason: decision.reason,
      evidence,
      liveTradesSincePromotion,
      history: history.slice(-20),
    };
  }

  state.updatedAt = now;
  writeJson(AUTO_PROMOTION_STATE_FILE, state);
  return state;
}

function autoPromotionFor(state: AutoPromotionState, setupId: string): AutoPromotionStateEntry | null {
  return state.families[setupId] ?? null;
}
...
function buildLlmTruthState(hypotheses: Hypothesis[], weights: SignalWeight[], closedTrades: ClosedTrade[], blockedSignals: BlockedSignalShadow[]): LlmTruthState {
  ...
  // Per-family P&L series, kept alongside the running averages so the truth
  // record can carry dispersion. Without it the status rule can only ask "how
  // often was this right", never "did it make money".
  const tradePnlSeries = new Map<string, number[]>();
  const shadowPnlSeries = new Map<string, number[]>();
  const pushPnl = (series: Map<string, number[]>, setupId: string, pnlPct: number): void => {
    if (!Number.isFinite(pnlPct)) return;
    const existing = series.get(setupId);
    if (existing) existing.push(pnlPct);
    else series.set(setupId, [pnlPct]);
  };
  ...
  for (const trade of closedTrades) {
    ...
    pushPnl(tradePnlSeries, setup.setupId, trade.pnlPct);
  ...
  for (const shadow of blockedSignals) {
    ...
    pushPnl(shadowPnlSeries, setup.setupId, shadow.hypotheticalResult.pnlPct);
    if (record.representativeExamples.length < 4) {
      record.representativeExamples.push({
        id: shadow.id,
        kind: "shadow",
  ...
  const setupFamilies = Array.from(records.values())
    .map((record) => {
      ...
      const tradeMoments = sampleMoments(tradePnlSeries.get(record.setupId) ?? []);
      const shadowMoments = sampleMoments(shadowPnlSeries.get(record.setupId) ?? []);
      if (tradeMoments.n > 1) record.evidenceSummary.tradePnlStdPct = Number(tradeMoments.std.toFixed(4));
      if (shadowMoments.n > 1) record.evidenceSummary.shadowPnlStdPct = Number(shadowMoments.std.toFixed(4));
...
function gateLlmAdvice(llmResult: LlmAnalysisResult | null, portfolio: Portfolio, candidateActions: CandidateActions): GatedLlmAdvice {
  return buildGatedLlmAdvice({
    llmResult,
    positions: portfolio.positions,
    llmCloseEligibility: candidateActions.llmCloseEligibility,
    allowHourlyLlmCloses: ALLOW_HOURLY_LLM_CLOSES,
  });
}
...
async function main() {
  ...
  const autoPromotions = evaluateAutoPromotions(blockedSignals, closedTrades);
  const gatedOneTouch = autoPromotionFor(autoPromotions, "one_touch_high_edge_no_gated");
  const oneTouchSizeFraction = autoPromotedSizeFraction(gatedOneTouch?.liveTradesSincePromotion ?? 0);
  const oneTouchHighEdgeNoLiveSignals = gatedOneTouch?.promoted
    ? generateOneTouchHighEdgeNoSignals(relativeValueRows, weights, learningParams, latestSnapshot, oneTouchSizeFraction)
    : [];
  const oneTouchHighEdgeLiveCoveredKeys = liveOneTouchHighEdgeNoKeys(oneTouchHighEdgeNoLiveSignals);
  for (const entry of Object.values(autoPromotions.families)) {
    console.log(entry.promoted
      ? `\n  Auto-promoted LIVE: ${entry.label} — ${entry.reason}`
      : `\n  Auto-promotion held: ${entry.label} — ${entry.reason}`);
  }
  if (oneTouchHighEdgeNoLiveSignals.length > 0) {
    console.log(`  Generated ${oneTouchHighEdgeNoLiveSignals.length} one-touch NO live signals (FIND-0020 gate, sell-YES edge >=${ONE_TOUCH_NO_SHADOW_MIN_ENTRY_EDGE_PTS}pt, size ${(oneTouchSizeFraction * 100).toFixed(0)}%).`);
  }
  ...
  if (newOneTouchHighEdgeShadows > 0) {
    console.log(`\n  Opened ${newOneTouchHighEdgeShadows} one-touch NO edge shadow trades${gatedOneTouch?.promoted ? "" : " (not yet auto-promoted; shadow-only)"}.`);
  }
  ...
  blockedObs = [...buildBlockedSignalObservations(blockedSummary, BLOCKED_SIGNAL_OBSERVATION_CONFIG), ...proxyComparisonObs, ...oneTouchBucketObs];
